...
function addTask(){
    ...
    newDiv.appendChild(newDetailsElement);// newDiv has 2 child nodes
    todos.push(newDiv.toString()); // array which stores all todos
    count++;
    ...
    tasksView[0].appendChild(todoContainer);//todo-container ->newDiv,iconEdit,iconDelete and newDiv has 2 child nodes
    iconDelete.addEventListener("click",function(){deleteThis(this);});
    iconEdit.addEventListener("click",function(){
      let j,input1,input2,input3,dateStatus,task= this.previousSibling;
      ...
      input3=dateStatus[1].slice(9); // status value will be returned
      document.getElementsByTagName("select")[0].value=input3;
      for(j=0;j<count;j++){
        if(todos[j].innerHTML==task.innerHTML){
      ...
      function()
      {
        ...
        todos[j]=inputs[0].value + `Date: ${inputs[1].value} <br> Status: ${inputs[2].value}`;
        task.firstChild.nodeValue= inputs[0].value;
        task.childNodes[1].innerHTML=`Date: ${inputs[1].value} <br> Status: ${inputs[2].value}`;
        setTimeout(
        function()
           {
             document.getElementsByClassName("form__submit")[0].addEventListener("click",addTask);},10000);
           });
       });
}
  function reset(){
